...
(function(){
    ...
    var Canvas = function(canvasWrap,id,width,height){
        ...
        var root = this,
            id,wrap,size,status,info,stopTimer,event_info = {};
        ...
        ************************************************************ */
        ...
        function init(){
            ...
            // create cavas
            root.$cvs = $('<canvas id="'+id+'" width="'+size.width+'" height="'+size.height+'"></canvas>');
            root.$cvs.appendTo(wrap);
            root.cvs = root.$cvs[0];
        ...
        ************************************************************ */
        ...
        this.mouseEventStart = function(){
            if(!this.info.mouseEvent){
                this.$cvs.bind('mousemove',mouseEventListener);
                this.$cvs.bind('mouseover',mouseEventListener);
                this.$cvs.bind('mouseout',mouseEventListener);
                this.info.mouseEvent = true;
            }
        };

        this.mouseEventStop = function(){
            if(this.info.mouseEvent){
                this.$cvs.unbind('mousemove',mouseEventListener);
                this.$cvs.unbind('mouseover',mouseEventListener);
                this.$cvs.unbind('mouseout',mouseEventListener);
            }
        }

        function mouseEventListener(e){
            switch(e.type){
                case 'mouseover'    : root.info.mouseStageOn = true; break;
                case 'mouseout'     : root.info.mouseStageOn = false; break;
                case 'mousemove'    : 
                    var rect = e.target.getBoundingClientRect();
                    root.mouse.x = e.clientX - rect.left;  
                    root.mouse.y = e.clientY - rect.top; 
                    break;
            }
        ...
        ************************************************************ */
        ...
        /* ************************************************************
            get
        ************************************************************ */
        this.getCanvas  = function(){ return this.cvs };
        ...
        ************************************************************ */
        ...
        this.getImageData = function(rect,original,callBack){
            var rectangle = {x:0,y:0,width:this.cvs.width,height:this.cvs.height};
            $.extend(rectangle,rect);
            var imgData = this.ctx.getImageData(rectangle.x, rectangle.y, rectangle.width, rectangle.height);
        ...
        this.getColorData = function(rect,dataCon,callBack){
            var rectangle   = {x:0,y:0,width:this.cvs.width,height:this.cvs.height};
            $.extend(rectangle,rect);

            var imgData     = this.ctx.getImageData(rectangle.x, rectangle.y, rectangle.width, rectangle.height),
                length      = imgData.width * imgData.height,
                data        = {positionData:[],standData:[],length:0};
            ...
            for (var i = 0 ;i < length; i++) {  
                ...
                if(a != 0){
                    ...
                    if(typeof data.positionData[x] == 'undefined')data.positionData[x] = [];
                    data.positionData[x][y] = {x:x,y:y,r:r,g:g,b:b,a:Number((a/255).toFixed(2))};
                    data.standData.push({x:x,y:y,r:r,g:g,b:b,a:Number((a/255).toFixed(2))});
                }

                if(i==length-1){
                    data.length = data.standData.length;

                    data.standData.sort(function(a, b){
                        var x1 = a.x,
                            x2 = b.x;
                        if (x1 > x2) return 1;
                        if (x1 < x2) return -1;
